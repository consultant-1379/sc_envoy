...
class RatelimitIntegrationTestBase : public HttpIntegrationTest {
public:
  RatelimitIntegrationTestBase(Http::CodecClient::Type downstream_protocol,
...
//        TestUtility::loadFromYaml(route_config, *hcm.mutable_route_config(), true);
  ...
  void compareJsonBodies(json rec, json expected);
protected:
};

class RatelimitIntegrationTestSsl : public RatelimitIntegrationTestBase {
public:
  RatelimitIntegrationTestSsl(Network::Address::IpVersion ip_version)
      : RatelimitIntegrationTestBase(Http::CodecClient::Type::HTTP1, ip_version) {}
