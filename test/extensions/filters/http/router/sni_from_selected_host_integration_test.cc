...
class SniFromSelectedHostIntegrationTest
    : public testing::TestWithParam<Network::Address::IpVersion>,
      public Event::TestUsingSimulatedTime,
      public HttpIntegrationTest {
public:
  SniFromSelectedHostIntegrationTest() : HttpIntegrationTest(Http::CodecType::HTTP1, GetParam()) {}
  ...
  void setup() {
    ...
    config_helper_.addConfigModifier([](envoy::config::bootstrap::v3::Bootstrap& bootstrap) {
      ...
      protocol_options.mutable_upstream_http_protocol_options()->set_enable_sni_from_host(true);
...
INSTANTIATE_TEST_SUITE_P(IpVersions, SniFromSelectedHostIntegrationTest,
                         testing::ValuesIn(TestEnvironment::getIpVersionsForTest()),
                         TestUtility::ipTestParamsToString);

TEST_P(SniFromSelectedHostIntegrationTest, BasicSniTest) {
  setup();
  ...
  EXPECT_STREQ("sepp.host.de", SSL_get_servername(ssl_socket->ssl(), TLSEXT_NAMETYPE_host_name));
}

TEST_P(SniFromSelectedHostIntegrationTest, SniAutoSniAndSelectedHost) {
  setup2();
  codec_client_ = makeHttpConnection(lookupPort("http"));
  const auto response_ = sendRequestAndWaitForResponse(
      Http::TestRequestHeaderMapImpl{
