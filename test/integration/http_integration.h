...
protected:
  ...
  // returns a string containing a counters printout, to be used along with ENVOY_LOG
  // @param starts_with only return counters starting with this string
  static std::string printCounters(Envoy::IntegrationTestServerPtr& test_server,
                                   absl::optional<std::string> starts_with = absl::nullopt);
