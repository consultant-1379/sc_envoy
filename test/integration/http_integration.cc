...
std::string HttpIntegrationTest::printCounters(Envoy::IntegrationTestServerPtr& test_server,
                                               absl::optional<std::string> starts_with) {
  std::string res = "counter_map = {";
  std::vector<Stats::CounterSharedPtr> counters = test_server->counters();
  for (auto& counter : counters) {
    if (starts_with.has_value() &&
        (counter->name().rfind(starts_with.value(), 0) != 0)) { // doesn't start with
      continue;
    }
    absl::StrAppend(&res, "\n", "{\"", counter->name(), "\", \"", counter->tagExtractedName(),
                    "\", \"", test_server->counter(counter->name())->value(), "\"},");
  }
  return res;
}
