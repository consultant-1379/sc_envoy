...
public:
  ...
  MOCK_METHOD(absl::optional<Upstream::LoadBalancerContext::OverrideHost>, upstreamOverrideHost, (),
              (const));
  MOCK_METHOD(void, setViaHeaderContents, (const std::vector<absl::string_view>&& parsed_cdns));
  MOCK_METHOD(const std::vector<absl::string_view>&, viaHeaderContents, (), (const));
  MOCK_METHOD(std::vector<uint32_t>, getPreferredHostRetryIndices, ());
  MOCK_METHOD(void, setPreferredHostRetryIndices, (const std::vector<uint32_t>&));
