...
public:
  ...
  // populates the provided struct* json_format with the contents of json.
  // Used by setAccessLog for json format
  static void configureJsonFormat(const nlohmann::json& json,
                                  google::protobuf::Struct* json_format);

  // Set the HTTP access log for the first HCM (if present) to a given file. The default is
  // the platform's null device. JSON format is used
  bool setAccessLog(const std::string& filename, const nlohmann::json& json_format,
