...
void ConfigHelper::configureJsonFormat(const nlohmann::json& json,
                                       google::protobuf::Struct* json_format) {
  for (const auto& [key, value] : json.items()) {
    if (value.is_string()) {
      json_format->mutable_fields()->operator[](key).set_string_value(value);
    } else if (value.is_structured()) {
      configureJsonFormat(value,
                          json_format->mutable_fields()->operator[](key).mutable_struct_value());
    }
  }
}

bool ConfigHelper::setAccessLog(
    const std::string& filename, const nlohmann::json& json, bool omit_empty_values,
    const envoy::config::accesslog::v3::AccessLogFilter& access_log_filter,
    std::vector<envoy::config::core::v3::TypedExtensionConfig> formatters) {
  if (getFilterFromListener("http") == nullptr) {
    return false;
  }
  ...
  if (!json.empty()) {
    auto* log_format = access_log_config.mutable_log_format();
    log_format->set_omit_empty_values(omit_empty_values);
    auto* json_format = log_format->mutable_json_format();
    configureJsonFormat(json, json_format);
  ...
  hcm_config.mutable_access_log(0)->mutable_filter()->MergeFrom(access_log_filter);
