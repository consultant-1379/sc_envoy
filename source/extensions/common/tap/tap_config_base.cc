...
#include "external/com_google_protobuf/src/google/protobuf/io/zero_copy_stream.h"
#include "external/com_google_protobuf/src/google/protobuf/io/zero_copy_stream_impl_lite.h"
...
TapConfigBaseImpl::TapConfigBaseImpl(const envoy::config::tap::v3::TapConfig& proto_config,
  ...
  case ProtoOutputSink::OutputSinkTypeCase::kFilePerTap:
    sink_ = std::make_unique<FilePerTapSink>(sinks[0].file_per_tap(),scope);
  ...
  case envoy::config::tap::v3::OutputSink::OutputSinkTypeCase::kStreamingGrpc:
    sink_ =
        std::make_unique<EricTapSink>(proto_config.output_config().sinks()[0].streaming_grpc(),scope);
    sink_to_use_ = sink_.get();
    break;
