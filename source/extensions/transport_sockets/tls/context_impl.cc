...
ContextImpl::ContextImpl(Stats::Scope& scope, const Envoy::Ssl::ContextConfig& config,
                         TimeSource& time_source, Ssl::KeyLoggerSharedPtr key_logger)
  ...
  for (size_t i = 0; i < tls_contexts_.size(); i++) {
    ...
    // DND-28793 SSL key logging:
    SSL_CTX_set_keylog_callback(ctx.ssl_ctx_.get(), osslKeylogCallback);
#endif
...
enum ssl_verify_result_t ContextImpl::customVerifyCallback(SSL* ssl, uint8_t* out_alert) {
  ...
  case ValidationResults::ValidationStatus::Failed: {
    ...
    ENVOY_LOG(trace,"Certificate Validation failed. ValidationResults::ErrorDetails:'{}'",result.error_details.value_or("NO_ERROR_DETAILS"));
...
#ifdef WITH_SSL_KEY_LOGGING
// DND28793 SSL Key logging
void ContextImpl::osslKeylogCallback(const SSL *ssl, const char *line)
{
  ContextImpl* context_impl =
    static_cast<ContextImpl*>(SSL_CTX_get_app_data(SSL_get_SSL_CTX(ssl)));
  if (context_impl && context_impl->key_logger_) {
    context_impl->key_logger_->logKey(line);
  }
}
#endif
...
ClientContextImpl::ClientContextImpl(Stats::Scope& scope,
                                     ...
                                     TimeSource& time_source, Ssl::KeyLoggerSharedPtr key_logger)
    : ContextImpl(scope, config, time_source, key_logger),
