...
ContextManagerImpl::ContextManagerImpl(TimeSource& time_source,
    AccessLog::AccessLogManager& access_log_manager)
    : time_source_(time_source) {
#ifdef WITH_SSL_KEY_LOGGING
  // The environment variable configures the filename/path of the key-logging
  // file or named pipe. If it is undefined, then it will not log any SSL keys.
  const char key_log_env_var_name[] = "TLS_KEYLOGGER_FIFO_PATH";
  char *key_log_file_name = getenv(key_log_env_var_name);
  key_logger_ = std::make_shared<KeyLoggerImpl>(access_log_manager, key_log_file_name);
#else
  (void)access_log_manager;
#endif
}
...
ContextManagerImpl::createSslClientContext(Stats::Scope& scope,
  ...
  Envoy::Ssl::ClientContextSharedPtr context =
      std::make_shared<ClientContextImpl>(scope, config, time_source_, key_logger_);
...
ContextManagerImpl::createSslServerContext(Stats::Scope& scope,
  ...
  Envoy::Ssl::ServerContextSharedPtr context =
      std::make_shared<ServerContextImpl>(scope, config, server_names, time_source_, key_logger_);
  contexts_.insert(context);
  return context;
}
