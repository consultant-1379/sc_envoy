...
Http::FilterHeadersStatus ProxyFilter::decodeHeaders(Http::RequestHeaderMap& headers, bool) {
  ...
  // DND-38571
  // Remove TaR header in DfP so that strict_dfp can work
  // with the same routing table as strict routing
  auto cb_filter_md = &decoder_callbacks_->streamInfo().dynamicMetadata().filter_metadata();
  if( cb_filter_md->contains("eric_proxy") &&
      cb_filter_md->find("eric_proxy")->second.fields().contains("dfp_remove_tar") &&
      cb_filter_md->find("eric_proxy")->second.fields().
        find("dfp_remove_tar")->second.string_value() == "true")                              
  {
    if(! headers.get(Http::LowerCaseString("3gpp-sbi-target-apiroot")).empty())
    {
