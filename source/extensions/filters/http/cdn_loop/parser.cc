...
StatusOr<ParseContext> parseToken(const ParseContext& input) {
  ...
  while (!context.atEnd()) {
    ...
    // SCDS-328: also move the dot to the head of the list
    if (isAlpha(c) || isDigit(c) || c == '.' || c == '-' || c == '_' || c == '!' || c == '#' || c == '$' ||
        c == '%' || c == '&' || c == '\'' || c == '*' || c == '+' || c == '^' ||
        c == '`' || c == '|' || c == '~'
        /* Begin SCDS-328 */ || c == '/' /* End SCDS-328 */) {
      context.increment();
    } else {
      break;
    }
...
StatusOr<ParsedCdnId> parseCdnId(const ParseContext& input) {
  ...
  if ((context.peek() == ' ') || (context.peek() == '\t')) {
    // If there is only whitespace coming, then the current position is the end
    // (= don't include trailing whitespace in the output)
    auto maybeEnd = context.next();
    context.setNext(skipOptionalWhitespace(context));
    if (context.atEnd()) {
      return ParsedCdnId(context,
                        context.value().substr(input.next(), maybeEnd - input.next()));
    }
...
StatusOr<ParsedCdnInfoList> parseCdnInfoList(const ParseContext& input) {
  ...
  while (!context.atEnd()) {
    ...
    if (StatusOr<ParsedCdnInfo> parsed_cdn_info = parseCdnInfo(context); !parsed_cdn_info.ok()) {
      ...
      // const auto& whitespace = parsed_cdn_info->cdnId().find(' ');
      // if (whitespace != absl::string_view::npos) {
      //   cdn_infos.push_back(parsed_cdn_info->cdnId().substr(whitespace + 1));
      // } else {
