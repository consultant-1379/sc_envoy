...
void Utility::responseFlagsToAccessLogResponseFlags(
  ...
  static_assert(StreamInfo::ResponseFlag::LastFlag == 0x80000000,
  ...
  // Envoy proxy flags Ericsson
  if (stream_info.hasResponseFlag(StreamInfo::ResponseFlag::AbsolutePathRejected)) {
    common_access_log.mutable_response_flags()->set_absolute_path_rejected(true);
  }

  if (stream_info.hasResponseFlag(StreamInfo::ResponseFlag::MissingPathRejected)) {
    common_access_log.mutable_response_flags()->set_missing_path_rejected(true);
  }

  if (stream_info.hasResponseFlag(StreamInfo::ResponseFlag::PayloadTooLarge)) {
    common_access_log.mutable_response_flags()->set_request_payload_too_large(true);
  }
}
