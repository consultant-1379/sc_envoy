...
struct MyFormatter {
  void operator()(std::string* out, std::reference_wrapper<Network::ListenerConfig> l) const {
    out->append(l.get().name());
  }
};
Http::Code ListenersHandler::handlerListenersReady(Http::ResponseHeaderMap&, Buffer::Instance&,
                                                   AdminStream&) {

  std::string listeners = absl::StrJoin(server_.listenerManager().listeners(), ",", MyFormatter());

  ENVOY_LOG_MISC(debug, "Currently active listeners: [{}]", listeners);
  if (server_.listenerManager().listeners().size() > 1) {
