...
public:
  ...
  static std::string logHeaders(const Http::RequestHeaderMap& headers);
  ...
  const Router::MetadataMatchCriteria* ericProxyMetadata() override {
    if (route_entry_) {
      ...
      // not change anymore.
      if (eric_proxy_metadata_ != nullptr) {
        return eric_proxy_metadata_.get();
      }
  ...
  bool shouldSelectAnotherHost(const Upstream::Host& host) override {
    ...
    // TODO: echaias: this removal only caters to a scenario for loop prevention. See if the retry
    // state/router
    // should somehow have knowledge of this
    // if (!is_retry_) {
    //   return false;
    // }
  ...
  std::vector<uint32_t> overrideHostRetryIndices() override {
    if( retry_state_ &&
        !callbacks_->getPreferredHostRetryIndices().empty() &&
        retry_state_->shouldRetryPreferredHost()) {
      return callbacks_->getPreferredHostRetryIndices();
    }
    return {0};
  }


  uint32_t getOverrideHostRetryIndexToChoose() override { 
      if(attempt_count_ == 1){
      return 0;
      } else if(!callbacks_->getPreferredHostRetryIndices().empty()){
        const auto& size = callbacks_->getPreferredHostRetryIndices().size();
        return ((last_chosen_retry_index_ + 1)%size);
      }
      return 0;
  }

  void setOverrideHostRetryIndexChosen(const uint32_t& last_index) override {
      last_chosen_retry_index_ = last_index;
  }
  ...
  const Upstream::HealthyAndDegradedLoad& determinePriorityLoad(
    ...
    // // The priority load is only modified on retries. However if this function is called
    // // it means we are done with overriden hosts from the hostmap
    // if (!is_retry_) {
  ...
  void finalizePreferredHostRetries() override {
    if (retry_state_) {
      retry_state_->finalizePreferredHostRetries();
    }
  }
  ...
  absl::optional<OverrideHost> overrideHostToSelect() const override {
    if (is_retry_ && !retry_state_->shouldRetryPreferredHost()) {
      return {};
    }
    return callbacks_->upstreamOverrideHost();
  }

  void setSniFromHostName(const absl::string_view server_name) override;

  void setAutoSanValidation(const absl::string_view server_name) override;
...
private:
  ...
  using EricProxyFilter = Extensions::HttpFilters::EricProxy::EricProxyFilter;
  ...
  inline void setPathHeaderAbsolute(const char* ulid);
  inline void setPathHeaderRelative();
  inline void directProcessing();
  inline void indirectProcessing();

  void addApiPrefixToPath(Upstream::HostDescriptionConstSharedPtr host, 
                            bool is_inidrect);

  // Only call analyzePrioritySet() when all endpoints of preferred host has been tried atleast once
  void analyzePrioritySet() {
    if (!lb_prio_set_analyzed_ && !retry_state_->shouldRetryPreferredHost()) {
      //ENVOY_LOG(trace, "Analyzing priority set for the first time (pref retries are 0)");
      const auto cluster = config_.cm_.getThreadLocalCluster(route_entry_->clusterName());
      if (cluster) {
        lb_prio_set_analyzed_ = true;

        cluster->loadBalancer().analyzePrioritySet(this);
      }
    }
  }

  // start remote-round-robin support (eedrak), implementation is in "/common/router/eric_proxy.cc"
  void saveDiscoveryHeadersToBePreserved(const ::google::protobuf::Map<std::string, ::google::protobuf::Struct>*); 
  void removeAllDiscoveryHeaders(); 
  void preserveDiscoveryHeaders(const ::google::protobuf::Map<std::string, ::google::protobuf::Struct>*);
  bool are_discovery_headers_preserved_ = false; 

  bool isDiscoveryHeader(const Http::HeaderEntry& header); 
  std::unique_ptr<Envoy::Http::RequestHeaderMapImpl> preserved_disc_headers_ = Envoy::Http::createHeaderMap<Envoy::Http::RequestHeaderMapImpl>({});
  std::shared_ptr<TarList> tar_list_; 
  ...
  void chargeUpstreamCodePerNf(uint64_t response_code, UpstreamRequest& upstream_request);
  void cleanup();
  ...
  createRetryState(const RetryPolicy& policy, Http::RequestHeaderMap& request_headers,
                   ...
                   const std::vector<absl::string_view>& via_header_hosts) PURE;
  ...
  MetadataMatchCriteriaConstPtr eric_proxy_metadata_;
  ...
  bool lb_prio_set_analyzed_{false};
  uint32_t last_chosen_retry_index_{0};
