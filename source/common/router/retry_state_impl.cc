...
RetryStateImpl::RetryStateImpl(const RetryPolicy& route_policy,
    ...
    : cluster_(cluster), vcluster_(vcluster), route_stats_context_(route_stats_context),
      ...
      auto_configured_for_http3_(auto_configured_for_http3), via_header_hosts_(via_header_hosts) {
...
RetryStatus RetryStateImpl::shouldRetry(RetryDecision would_retry, DoRetryCallback callback) {
  ...
  // (or the x-envoy-max-retries header) or there are no other priorities left to try
  if (!areRetriesAndPrioritiesRemaining()) {
...
RetryStatus RetryStateImpl::shouldRetryHeaders(const Http::ResponseHeaderMap& response_headers,
  ...
  // Yes, we will retry based on the headers - try to parse a rate limited reset interval from
  // the response.
...
bool RetryStateImpl::areRetriesAndPrioritiesRemaining() {
  if (retry_priority_) {
    return retries_remaining_ > 0 && retry_priority_->shouldRetry();
  }

  return retries_remaining_ > 0;
}
