...
public:
  ...
  //updated following method for use of upstream filters with Envoy v1.27.0 (eedrak)
  void setBufferedRequestBody(Buffer::Instance& buffer_data) {
    const auto& fm_buffered_rq_data = filter_manager_->bufferedRequestData();
    if (fm_buffered_rq_data != nullptr) {
      if (fm_buffered_rq_data->length() > 0) {
        fm_buffered_rq_data->drain(fm_buffered_rq_data->length());
      }
      fm_buffered_rq_data->move(buffer_data);
    }
  }
