...
public:
  ...
  bool shouldRetryPreferredHost() override {
    return (retry_priority_ && retry_priority_->preferredHostRetries() > 0);
  }

  void finalizePreferredHostRetries() override {
    if (retry_priority_) {
      retry_priority_->preferredHostRetries() = 0;
    }
  }

  bool shouldSelectAnotherHost(const Upstream::Host& host) override {

    return std::any_of(retry_host_predicates_.begin(), retry_host_predicates_.end(),
                       [&host, this](auto predicate) {
                         return predicate->shouldSelectAnotherHost(host, via_header_hosts_);
                       });
  }
  ...
  const Upstream::HealthyAndDegradedLoad& priorityLoadForRetry(
    ...
    return retry_priority_->determinePriorityLoad(priority_set, original_priority_load,
                                                  priority_mapping_func, via_header_hosts_);
  }
...
private:
  ...
  bool areRetriesAndPrioritiesRemaining();
  ...
  const std::vector<absl::string_view>& via_header_hosts_;
};
