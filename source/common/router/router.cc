...
void Filter::setSniFromHostName(const absl::string_view server_name) {
...
void Filter::setAutoSanValidation(const absl::string_view server_name) {
  if (!callbacks_->streamInfo().filterState()->hasDataWithName(
          Network::UpstreamSubjectAltNames::key())) {
    callbacks_->streamInfo().filterState()->setData(
        Network::UpstreamSubjectAltNames::key(),
        std::make_unique<Network::UpstreamSubjectAltNames>(
            std::vector<std::string>{std::string(server_name)}),
        StreamInfo::FilterState::StateType::Mutable);
  }
}
...
void Filter::chargeUpstreamCodePerNf(uint64_t response_code, UpstreamRequest& upstream_request){
  if (upstream_request.upstreamHost()) {
    switch (response_code / 100) {
    case 1:
      upstream_request.upstreamHost()->statsPerNf().upstreamRq1xxPerNfInc();
      break;
    case 2:
      upstream_request.upstreamHost()->statsPerNf().upstreamRq2xxPerNfInc();
      break;
    case 3:
      upstream_request.upstreamHost()->statsPerNf().upstreamRq3xxPerNfInc();
      break;
    case 4:
      upstream_request.upstreamHost()->statsPerNf().upstreamRq4xxPerNfInc();
      break;
    case 5:
      upstream_request.upstreamHost()->statsPerNf().upstreamRq5xxPerNfInc();
      break;
    default:
      ENVOY_STREAM_LOG(debug, "Could not find matching response code class for response code '{}' to step the respective per_nf counter.", *callbacks_, response_code);
      break;
    }
  }
}
...
Http::FilterHeadersStatus Filter::decodeHeaders(Http::RequestHeaderMap& headers, bool end_stream) {
  ...
  // TODO: enable this after adapting the manager to provide a name for every route
  // ENVOY_STREAM_LOG(debug, "Using Envoy route '{}'", *callbacks_, route_entry_->routeName());
  ...
  // start remote-round-robin support (eedrak)
  // set up TarList remote-round-robin
  auto cb_filter_md = &callbacks_->streamInfo().dynamicMetadata().filter_metadata();
  if ((EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy", "target-api-root-processing",
                                          "true")) &&
      (EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy", "target-api-root-values"))) {
    // TODO : use unique_ptr if need be or a regular pointer would do
    // No need for a shared pointer 
    // what is the rationale ??
    // Move it to run_ctx_ and use it in router.cc using filter-state-objects
    tar_list_ = std::make_shared<TarList>(callbacks_);
    tar_list_->setTarValuesFromMd(cb_filter_md);
  }

  // set up preserve-disc-params remote-round-robin (eedrak)
  if ((Extensions::HttpFilters::EricProxy::EricProxyFilter::findInDynMetadata(
    ...
    saveDiscoveryHeadersToBePreserved(cb_filter_md);
  }
  // end remote-round-robin support (eedrak)
  ...
  // moved up the creation of retry state so that it's there before the first try
  retry_state_ = createRetryState(route_entry_->retryPolicy(), headers, *cluster_,
                                  ...
                                  route_entry_->priority(), callbacks_->viaHeaderContents());
...
void Filter::onResponseTimeout() {
  ...
  while (!upstream_requests_.empty()) {
    ...
    if (upstream_request->upstreamHost()) {
      ...
      upstream_request->upstreamHost()->statsPerNf().upstreamRqTimeoutPerNfInc();
    }
...
void Filter::directProcessing() {
  // ULID(R24)
  ENVOY_STREAM_UL_LOG(debug, "Applying direct processing.", *callbacks_, ULID(R24));
  ProtobufWkt::Struct metadata;
  *(*metadata.mutable_fields())["direct-or-indirect"].mutable_string_value() = "direct";
  callbacks_->streamInfo().setDynamicMetadata("eric_proxy", metadata);

  // ULID(R06)
  auto cb_filter_md = &callbacks_->streamInfo().dynamicMetadata().filter_metadata();
  if (EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy", "absolute-path-processing",
                                         "true")) {
    setPathHeaderRelative();
  }

  // ULID(R09)
  ENVOY_STREAM_UL_LOG(debug, "Removing Target-Api-Root header.", *callbacks_, ULID(R09));
  downstream_headers_->remove(Http::LowerCaseString("3gpp-Sbi-target-apiRoot"));

  // ULID(R34)  In directProcessing we should unconditionally remove all discovery headers
  removeAllDiscoveryHeaders();
}

void Filter::indirectProcessing() {
  // ULID(R25) Decide if a Target-apiRoot header has to be added to the response:
  ENVOY_STREAM_UL_LOG(debug, "Applying indirect processing.", *callbacks_, ULID(R25));
  ProtobufWkt::Struct metadata;
  *(*metadata.mutable_fields())["direct-or-indirect"].mutable_string_value() = "indirect";
  callbacks_->streamInfo().setDynamicMetadata("eric_proxy", metadata);
  ...
  if (EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy", "absolute-path-processing",
                                         "true")) {
    setPathHeaderAbsolute(ULID(R16));
  }

  // When indirect routing to an NF Producer via a proxy 
  // SCP is a release 16 NF so it by default supports TaR
  // SEPP could have a TaR or No-TaR variant (Do we support such routing SCP->SEPP / SEPP->SCP)
  // when node type = SCP/SEPP
  // If peer SEPP supports TaR : Set 3gpp-Sbi-Target-apiRoot = <scheme>://<authority>/{apiRoot of peer NF}
  //                           : Set :path = <api-root-of-sepp>/<remaining-path>
  // If peer SEPP doesnt support TaR: Set :path = <api-root-of-sepp>/<remaining-path>
  // ULID(R17)
  if (EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy", "target-api-root-processing",
                                         "true")) {
      if (EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy", "target-api-root-value")) {
        // ULID(R18)
        ENVOY_STREAM_UL_LOG(debug, "Setting 3gpp-Sbi-target-apiRoot header from Dynamic Metadata.",
                         *callbacks_, ULID(R18));
        downstream_headers_->setCopy(Http::LowerCaseString("3gpp-Sbi-target-apiRoot"),
                                     EricProxyFilter::extractFromDynMetadata(
                                         cb_filter_md, "eric_proxy", "target-api-root-value"));
      } else if (EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy",
                                                    "target-api-root-values")) {
        // ULID(R31)  This is "remote-round-robin"
        ENVOY_STREAM_UL_LOG(trace, "inserting the next value to the  3gpp-Sbi-target-apiRoot header.",
                         *callbacks_, ULID(R31));
        downstream_headers_->setCopy(Http::LowerCaseString("3gpp-Sbi-target-apiRoot"),
                                     tar_list_->getNextTarValue());
      } else {
        ENVOY_STREAM_UL_LOG(debug, "Removed Target-Api-Root header.", *callbacks_, ULID(R32));
          downstream_headers_->remove(Http::LowerCaseString("3gpp-Sbi-target-apiRoot"));
      }
  } else {
    ENVOY_STREAM_UL_LOG(debug, "Removed Target-Api-Root header.", *callbacks_, ULID(R22));
      downstream_headers_->remove(Http::LowerCaseString("3gpp-Sbi-target-apiRoot"));
  }

  // ULID(R33)  Preserve discovery parameter handling
  if (EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy",
                                         "disc-parameters-to-be-preserved-if-indirect") ||
      EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy",
                                         "preserve-all-disc-parameters-if-indirect")) {
    preserveDiscoveryHeaders(cb_filter_md);
  } else {
    removeAllDiscoveryHeaders();
  }
}
...
// ULID(R07)
void Filter::setPathHeaderRelative() {
  auto cb_filter_md = &callbacks_->streamInfo().dynamicMetadata().filter_metadata();
  // switch (path_type) {
  // // For Direct Processing
  // case Relative: {
    if (EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy", "relative-path-value")) {
      ENVOY_STREAM_LOG(debug, "Setting :path header to relative path value.", *callbacks_);
      if (!downstream_headers_->get(Http::LowerCaseString(":path")).empty()) {
        downstream_headers_->remove(Http::LowerCaseString(":path"));
      }
      downstream_headers_->setCopy(Http::LowerCaseString(":path"),
                                   EricProxyFilter::extractFromDynMetadata(
                                       cb_filter_md, "eric_proxy", "relative-path-value"));
    }
  // }
  // // For Indirect Processing
  // case Absolute: {
  //   if (EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy", "absolute-path-value")) {
  //     ENVOY_STREAM_LOG(debug, "Setting :path header to absolute path value.", *callbacks_);

  // if (EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy", "relative-path-value")) {
  //   ENVOY_STREAM_UL_LOG(debug, "Setting :path header to relative path value.", *callbacks_, ULID(R07));
  //   downstream_headers_->setCopy(
  //       Http::LowerCaseString(":path"),
  //       EricProxyFilter::extractFromDynMetadata(cb_filter_md, "eric_proxy", "relative-path-value"));
  // }
}

// Note the flag s_retry parameter can be cleaned out when last-host Dyn MD moves
// to a filter state object
void Filter::addApiPrefixToPath(Upstream::HostDescriptionConstSharedPtr host,
                                bool is_indirect) {
  // TODO Get a UL ID ??
  const StreamInfo::FilterStateSharedPtr& filter_state =
    callbacks_->streamInfo().filterState();
  const StreamInfo::EricProxyState* state_info =
    filter_state->getDataReadOnly<StreamInfo::EricProxyState>(
        StreamInfo::EricProxyState::key());
  if(state_info != nullptr ) {
    // Do not execute at Notify + direct + (preferred-host+try)
    // Execute at reselections for all request types
    // TODO(ULID)
    // Have to support pref_host_retry = 0 as well
    // (TODO) Maybe better option is to have a custom onUpstreamHostReselected()
    // callback registered that would be invoked on a reselection and this part can entirely
    // be moved there without many conditions
    // If upstreamOverrideHost != host->hostName() we are directly on reselects at attempt_count = 1 so
    // add ApiPrefix
    // Do not execute at Notify + direct + (preferred-host+try)
    // Execute at reselections for all request types
    // TODO(ULID)
    bool is_direct_notify = ((!is_indirect) && state_info->getEricRunContextPtr()->getServiceClassifierCtx().isNotify());
    bool should_retry_pref_host = retry_state_ && retry_state_->shouldRetryPreferredHost();
    bool should_not_retry_pref_host = (retry_state_ && !retry_state_->shouldRetryPreferredHost());
    // Never execute on first try of direct notify
    if(!(is_direct_notify && attempt_count_ == 1 && callbacks_->upstreamOverrideHost().has_value() && absl::EqualsIgnoreCase(callbacks_->upstreamOverrideHost().value().first, host->hostname()))) {
      if( (should_not_retry_pref_host)  ||                                  // on reselect always execute
          ( should_retry_pref_host && attempt_count_ == 1 && !(is_direct_notify )  )          // Only on first try of preferred or strict routing -
            // when we have service direct/indirect request OR indirect notifications
        ) {
        const auto& val = Envoy::Config::Metadata::metadataValue(host->metadata().get(),"envoy.eric_proxy","prefix");
        if(val.has_string_value()){
          const auto& path = absl::StrCat(val.string_value(),downstream_headers_->Path()->value().getStringView());
          downstream_headers_->setPath(path);
          //Set Api prefix of selected host in Run Ctx 
          state_info->getEricRunContextPtr()->setSelectedHostApiPrefix(val.string_value());
          ENVOY_STREAM_LOG(debug,"Setting the path header to:'{}'",*callbacks_,path);
        }
      } else {
        ENVOY_STREAM_LOG(debug,"Not modifying the path header on conditions Is notify:'{}',Is indirect Request:'{}', attempt_count:'{}', shouldRetryPreferredHost():'{}'",
                                  *callbacks_, state_info->getEricRunContextPtr()->getServiceClassifierCtx().isNotify(),
                                  is_indirect,attempt_count_, (retry_state_ && retry_state_->shouldRetryPreferredHost() > 0));
      }
    } else {
      ENVOY_STREAM_LOG(debug, "Not adding api prefix on first try of direct notification",*callbacks_);
    }
  }
}

void Filter::setPathHeaderAbsolute(const char* ulid) {
  auto cb_filter_md = &callbacks_->streamInfo().dynamicMetadata().filter_metadata();

  if (EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy", "absolute-path-value")) {
    ENVOY_STREAM_UL_LOG(debug, "Setting :path header to absolute path value.", *callbacks_, ulid);
    downstream_headers_->setCopy(
        Http::LowerCaseString(":path"),
        EricProxyFilter::extractFromDynMetadata(cb_filter_md, "eric_proxy", "absolute-path-value"));
  }
}


// A host has been selected. Apply processing for:
// - nf-peer-info header
// - per_nf counter handling
// - Target-Api-Root
// - TFQDN Processing
// TODO[]: rewrite metadata handling:
// TODO[]: 1) use iterators directly without findInDynMetadata on addresses
// TODO[]: 2) filter_metadata already returns lvalue & do not need to take an adress
void Filter::onUpstreamHostSelected(Upstream::HostDescriptionConstSharedPtr host,
                                    bool pool_success) {
  ENVOY_STREAM_LOG(trace, "onUpstreamHostSelected()", *callbacks_);
  // ### DND-24889 && DND-27601 ###
  if (host) {
    if ((host->metadata() != nullptr)) {
      // ULID(R28) NF-PeerInfo Handling
      const auto sbi_nf_peer_info_pr =
          std::make_unique<Extensions::HttpFilters::EricProxy::SbiNfPeerInfoHeaderRequestMetadata>(
              callbacks_->streamInfo().dynamicMetadata(), host->metadata());
      if (sbi_nf_peer_info_pr->isActivated()) {
        sbi_nf_peer_info_pr->setSelectedFqdn(host->hostname());
        sbi_nf_peer_info_pr->setAll(*downstream_headers_);
        sbi_nf_peer_info_pr->saveUpdatedHeaderInMetadata(callbacks_, *downstream_headers_);
      }
      const StreamInfo::FilterStateSharedPtr& filter_state =
          callbacks_->streamInfo().filterState();
      const StreamInfo::EricProxyState* eric_proxy_state =
          filter_state->getDataReadOnly<StreamInfo::EricProxyState>(
              StreamInfo::EricProxyState::key());

      auto cb_filter_md = &callbacks_->streamInfo().dynamicMetadata().filter_metadata();

      // ULID(R29) Per NF counters
      host->statsPerNf().upstreamRqTotalPerNfInc();
      
      // Step per NF counters for retries/reselect based on last-host dyn-MD
      // before it is updated in next step R27
      // DND 59215 Api Prefix support
      // Step per NF counters for retries/reselect based on last-host dyn-MD before it is updated in next step R27
      if(eric_proxy_state && eric_proxy_state->getEricRunContextPtr() && 
            !eric_proxy_state->getEricRunContextPtr()->getSelectedHostAuthority().empty()) {                                        
        if(!host->hostname().empty()) {
          if(absl::EqualsIgnoreCase(eric_proxy_state->getEricRunContextPtr()->getSelectedHostAuthority(),
                host->hostname()) ){
            // current call is a retry
            // step per nf counter for retries
            ENVOY_STREAM_LOG(trace, "Stepping _per_nf counter for retry",*callbacks_);
            host->statsPerNf().upstreamRqAfterRetryPerNfInc();
          } else {
            // step per nf counter for reselects
            ENVOY_STREAM_LOG(trace, "Stepping _per_nf counter for reselect",*callbacks_);
            host->statsPerNf().upstreamRqAfterReselectPerNfInc();
          }
        }
      } 
      
      // ULID [R27]
      // Update "selected_host_" attribute in run context to point to the last selected host for this request
      // Used on response path by eric_proxy to decide if a TaR should be included on the response
      if(eric_proxy_state != nullptr && eric_proxy_state->getEricRunContextPtr() != nullptr){
        ENVOY_LOG(debug,"Storing last-host auth & api prefix in eric_proxy state run_ctx");
        eric_proxy_state->getEricRunContextPtr()->setSelectedHostAuthority(host->hostname());
        }
      

      // Set (= very first host selected) or Update "last-host" dyn-MD to point to the last
      // selected host for this request.
      // Used here (a few lines above) to step per-nf counters (detection if initial request or
      //           retry/reselect),
      // and used on response path by eric_proxy to decide if a TaR should be included on the response.
      // ProtobufWkt::Struct metadata;
      // *(*metadata.mutable_fields())["last-host"].mutable_string_value() = host->hostname();
      // callbacks_->streamInfo().setDynamicMetadata("eric_proxy", metadata);

      // ULID(R00)
      if (EricProxyFilter::findInDynMetadata(&host->metadata()->filter_metadata(),
                                             "envoy.eric_proxy")) {
        const auto envoy_eric_proxy = host->metadata()->filter_metadata().find("envoy.eric_proxy");
        ASSERT(envoy_eric_proxy != host->metadata()->filter_metadata().end());

        if (EricProxyFilter::findInDynMetadata(&host->metadata()->filter_metadata(),
                                               "envoy.eric_proxy", "support")) {
          const auto support = envoy_eric_proxy->second.fields().find("support");
          if (support->second.has_list_value()) {
            const std::string& support_val = support->second.list_value().values(0).string_value();
            ProtobufWkt::Struct metadata;
            *(*metadata.mutable_fields())["support"].mutable_string_value() = support_val;
            callbacks_->streamInfo().setDynamicMetadata("eric_proxy.sepp.routing", metadata);
            ENVOY_STREAM_UL_LOG(debug, "Support metadata is set to {}.", *callbacks_, ULID(R00),
                                support_val);

            if (support_val == "Indirect") {
              // ULID(R30) Indirect processing, but is it with or without TaR header?
              if (EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy",
                                                     "keep-authority-header", "true")) {
                // ULID(R19) Indirect processing without TaR header -> set preferred host
                // in :authority header
                if (EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy",
                                                       "absolute-path-processing", "true")) {
                  // ULID(R20) set abs. path from dyn MD "absolute-path-value"
                  setPathHeaderAbsolute(ULID(R20));
                }

                // ULID(R21)
                ENVOY_STREAM_UL_LOG(debug, "Removing Target-Api-Root header.", *callbacks_, ULID(R21));
                  downstream_headers_->remove(Http::LowerCaseString("3gpp-Sbi-target-apiRoot"));

                if (EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy",
                                                       "preferred-host")) {
                  // ULID(R14)
                  auto preferred_host = EricProxyFilter::extractFromDynMetadata(
                      cb_filter_md, "eric_proxy", "preferred-host");
                  ENVOY_STREAM_UL_LOG(
                      debug, "Setting the authority header to the preferred host:port -> {}.",
                      *callbacks_, ULID(R14), preferred_host);
                  downstream_headers_->setCopy(Http::LowerCaseString(":authority"), preferred_host);
                }
              } else {
                // ULID(R25) Indirect processing with TaR header containing the preferred host
                indirectProcessing();
                // DND 59215 : Set the :path header with a prefix which is the api prefix of the chosen host
                // Add the api root of peer proxy after :path header has been set in setPathHeader()
                // call in directProcessing() or indirectProcessing()
                // Only do the following on the first host selection attempt_count_ = 1 OR
                // on reselections to different hosts
                // ULID(R41)
                addApiPrefixToPath(host, true);

                // ULID(R13)
                ENVOY_STREAM_UL_LOG(debug, "Setting the authority header to hostname:port -> {}",
                                    *callbacks_, ULID(R13), host->hostname());
                downstream_headers_->setCopy(Http::LowerCaseString(":authority"), host->hostname());
              }
            } else if (support_val == "TFQDN" || support_val == "NF") {
              // ULID(R24) - ULID(34)
              directProcessing();
              // DND 59215
              // ULID(R42)
              addApiPrefixToPath(host, false);
              // ULID [R11] (if NF), ULID [R12] (if TFQDN)
              absl::string_view auth_val;
              bool interplmn_fqdn_found{false};
              if (eric_proxy_state &&
                  !eric_proxy_state
                       ->getStrValueForKey(StreamInfo::EricProxyState::StringValues::InterplmnFqdn)
                       .empty()) {
                auth_val = eric_proxy_state->getStrValueForKey(
                    StreamInfo::EricProxyState::StringValues::InterplmnFqdn);
                interplmn_fqdn_found = !interplmn_fqdn_found;
              } else {
                auth_val = host->hostname();
              }
              ENVOY_STREAM_UL_LOG(debug, "Setting the authority header to {}:port -> {}",
                                  *callbacks_, ULID(R11),
                                  interplmn_fqdn_found ? "interplmnfqdn" : "hostname", auth_val);
              downstream_headers_->setCopy(Http::LowerCaseString(":authority"), auth_val);
            } else {
              ENVOY_STREAM_UL_LOG(debug,
                                  "Support metadata not one of the following: NF, TFQDN, Indirect.",
                                  *callbacks_, ULID(R00b));
            }

            // ### DND-27601 ###
            // TFQDN Body processing
            const StreamInfo::FilterStateSharedPtr& filter_state =
                callbacks_->streamInfo().filterState();
            StreamInfo::EricProxySeppState* eric_sepp_state_ptr =
                filter_state->getDataMutable<StreamInfo::EricProxySeppState>(
                    StreamInfo::EricProxySeppState::key());
            if (support_val == "TFQDN") {
              // ULID(R35)
              if (eric_sepp_state_ptr && eric_sepp_state_ptr->getRoutingDirection() ==
                                             StreamInfo::EricProxySeppState::ExtToInt) {
                // Replace Body with modified body and set correct content-length
                // Save original body in Dynamic Metadata.
                // Only if the body is modified and not already in TFQDN format.
                if (!eric_sepp_state_ptr->getModifiedBody().empty() &&
                    eric_sepp_state_ptr->getModifiedBodyLen() != 0 &&
                    !eric_sepp_state_ptr->wasBodyModifiedForTfqdn()) {
                  ENVOY_STREAM_UL_LOG(debug, "Replacing the body with the modified body.",
                                   *callbacks_, ULID(R36));

                  auto orig_body_str = callbacks_->decodingBuffer()->toString();
                  eric_sepp_state_ptr->setOriginalBody(std::move(orig_body_str));
                  eric_sepp_state_ptr->setBodyModifiedForTqfdnFlag(true);
                  const auto sepp_tfqdn_modified_body = eric_sepp_state_ptr->getModifiedBody();

                  callbacks_->modifyDecodingBuffer(
                      [sepp_tfqdn_modified_body](Buffer::Instance& data) {
                        data.drain(data.length());
                        data.add(sepp_tfqdn_modified_body);
                      });

                  const auto sepp_tfqdn_modified_body_len =
                      eric_sepp_state_ptr->getModifiedBodyLen();
                  downstream_headers_->setCopy(Http::LowerCaseString("content-length"),
                                               std::to_string(sepp_tfqdn_modified_body_len));

                  if (!upstream_requests_.empty() && upstream_requests_.front()) {
                    Buffer::OwnedImpl modified_body(sepp_tfqdn_modified_body);
                    upstream_requests_.front()->setBufferedRequestBody(modified_body);
                  }
                } else {
                  ENVOY_STREAM_UL_LOG(debug, "Doing nothing with the body.", *callbacks_, ULID(R36b));
                }
              }
            } else if (support_val == "NF" ||
                       (support_val == "Indirect" &&
                        !EricProxyFilter::findInDynMetadata(cb_filter_md, "eric_proxy",
                                                            "keep-authority-header", "true"))) {
              // ULID(R37)
              if (eric_sepp_state_ptr && eric_sepp_state_ptr->wasBodyModifiedForTfqdn() &&
                  eric_sepp_state_ptr->getOriginalBodyLen() != 0) {
                ENVOY_STREAM_UL_LOG(debug,
                                 "Original body was modified. Restoring now the original body.",
                                 *callbacks_, ULID(R38));

                const auto sepp_tfqdn_original_body = eric_sepp_state_ptr->getOriginalBody();
                const auto sepp_tfqdn_original_body_len = eric_sepp_state_ptr->getOriginalBodyLen();

                callbacks_->modifyDecodingBuffer(
                    [sepp_tfqdn_original_body](Buffer::Instance& data) {
                      data.drain(data.length());
                      data.add(sepp_tfqdn_original_body);
                    });

                downstream_headers_->setCopy(Http::LowerCaseString("content-length"),
                                             std::to_string(sepp_tfqdn_original_body_len));

                if (!upstream_requests_.empty() && upstream_requests_.front()) {
                  Buffer::OwnedImpl original_body(sepp_tfqdn_original_body);
                  upstream_requests_.front()->setBufferedRequestBody(original_body);
                }

                eric_sepp_state_ptr->setBodyModifiedForTqfdnFlag(false);
              } else {
                ENVOY_STREAM_UL_LOG(debug, "Original body was not replaced. Doing nothing.",
                                 *callbacks_, ULID(R38b));
              }
            }
          } else {
            ENVOY_STREAM_UL_LOG(debug, "Support metadata not set in the config.", *callbacks_, ULID(R00b));
          }
        } else {
          ENVOY_STREAM_UL_LOG(debug, "Support metadata not present in the config.", *callbacks_, ULID(R00c));
        }

        // ULID(R39)  TS 29500 Sbi-Producer-Id handling on host selection/reselection
        // delegation to SCP
        if (EricProxyFilter::findInDynMetadata(&host->metadata()->filter_metadata(),
                                               "envoy.eric_proxy", "nfInstanceId")) {
          const auto producer_ctx = envoy_eric_proxy->second.fields().find("nfInstanceId");
          if (producer_ctx != envoy_eric_proxy->second.fields().end()) {
            // Put the selected host's nFInst Id into nf-inst-id MD in eric_proxy
            // nfInstanceId MD has to be a array because rxgrpc-envoy-api has MD only as a
            // Map<String,List<String>> and not Map<String,String>
            if (producer_ctx->second.has_list_value()) {
              const auto& producer_ctx_size = producer_ctx->second.list_value().values_size();
              std::vector<std::string> md_vec = {"nf-inst-id", "nf-serv-inst-id", "nf-set-id",
                                                 "nf-serv-set-id"};
              for (int i = 0; i < producer_ctx_size; i++) {
                ProtobufWkt::Struct producer_ctx_md;
                *(*producer_ctx_md.mutable_fields())[md_vec[i]].mutable_string_value() =
                    producer_ctx->second.list_value().values(i).string_value();
                callbacks_->streamInfo().setDynamicMetadata("eric_proxy", producer_ctx_md);
              }
            }
          } else {
            ENVOY_STREAM_UL_LOG(debug, "NFInstId MD not present for the host:{}", *callbacks_,
                                ULID(R39), host->hostname());
          }
        } else {
          ENVOY_STREAM_UL_LOG(debug, "NFInstId MD not present for the host:{}", *callbacks_,
                              ULID(R39), host->hostname());
      ...
      // ULID(R40)  DND 37752 Modify scheme in case of mixed cluster
      // on a host selection/reselection based on envoy.transport_socket_match MD
      if (EricProxyFilter::findInDynMetadata(&host->metadata()->filter_metadata(),
                                             "envoy.transport_socket_match", "matchTLS") &&
            eric_proxy_state && eric_proxy_state->getEricRunContextPtr() != nullptr ) {
        ENVOY_STREAM_UL_LOG(debug, "Altering scheme ...", *callbacks_, ULID(R40));
        const auto is_https = host->metadata()
                                  ->filter_metadata()
                                  .find("envoy.transport_socket_match")
                                  ->second.fields()
                                  .find("matchTLS")
                                  ->second.list_value()
                                  .values(0)
                                  .string_value() == "true" ? true : false;

        // Ensure that new test cases if they wish to validate some form of
        // upstream :scheme or downstream indications of upstream scheme they need
        // to add matchTLS key MD to "true" or "false" in envoy.transport_socket_match
        // namespace, its easier to do with EndPointConfigurator::withScheme() or 
        // just add an MD manually
        is_https ? 
        eric_proxy_state->getEricRunContextPtr()
                          ->setSelectedHostScheme(Extensions::HttpFilters::EricProxy::RunContext::UpstreamHostScheme::Https)
                          :
        eric_proxy_state->getEricRunContextPtr()
                          ->setSelectedHostScheme(Extensions::HttpFilters::EricProxy::RunContext::UpstreamHostScheme::Http);

        downstream_headers_->setScheme(is_https ? "https":"http");
      }
    }
  }
...
void Filter::onUpstream1xxHeaders(Http::ResponseHeaderMapPtr&& headers,
  ...
  // Step per_nf counter for respective response code class
  chargeUpstreamCodePerNf(response_code, upstream_request);
...
void Filter::onUpstreamHeaders(uint64_t response_code, Http::ResponseHeaderMapPtr&& headers,
                               UpstreamRequest& upstream_request, bool end_stream) {
  ENVOY_STREAM_LOG(debug, "upstream headers complete: response_code:'{}', end_stream={}",
                   *callbacks_, response_code, end_stream);
  ...
  if (retry_state_) {
    if (upstream_request.retried()) {
      ...
      // analyzePrio Set after all preferred host retry attempts have been attempted
      // for all endpoints
      analyzePrioritySet();
      ...
      if (retry_status == RetryStatus::Yes) {
        // DND-59215 Support for Api Prefix
        // After the first response and deciding that we 
        // need to retry/reselect the request
        // Modify the path header by stripping the api-prefix of 
        // the currently selected host if it matches the apiPrefix mentioned
        // in the endpoint MD 
        // Only strip prefix on a reselection which happens when shouldRetryPreferredHost() is false
        // ULID(R43)
        if(retry_state_ && !retry_state_->shouldRetryPreferredHost()) {
            const ProtobufWkt::Value& val = Envoy::Config::Metadata::metadataValue(
                upstream_request.upstreamHost()->metadata().get(),"envoy.eric_proxy","prefix");
          if(val.has_string_value()) {
            const auto& api_prefix = val.string_value();
            const auto path_hdr = downstream_headers_->Path()->value().getStringView();
            if(absl::StartsWith(path_hdr,api_prefix)){
              // Strip the prefix 
              // Remove Path and create new Path 
              // with stripped prefix 
              // TODO(ULID)
              ENVOY_STREAM_LOG(debug,"Stripping prefix:'{}' from :path header",*callbacks_,api_prefix);
              downstream_headers_->setPath(absl::StripPrefix(path_hdr,api_prefix));
            } 
          }
          // else dont strip the api-prefix
          // and on reselection add the prefix of the 
          // reselected host in path header in onUpstreamHostSelected()
        }
...
ProdFilter::createRetryState(const RetryPolicy& policy, Http::RequestHeaderMap& request_headers,
  ...
  std::unique_ptr<RetryStateImpl> retry_state =
      RetryStateImpl::create(policy, request_headers, cluster, vcluster, route_stats_context,
                             runtime, random, dispatcher, time_source, priority, via_header_hosts);
