...
// An eric event formatter that extracts event related fields from EricEventState stored in the
// filter state. Must be provided with an access log command that %EVENT(<param>)%
EventFormatter::EventFormatter(const std::string& field, absl::optional<size_t> max_length)
    : field_(field), max_length_(max_length) {}

absl::optional<std::string>
EventFormatter::format(const StreamInfo::StreamInfo& stream_info) const {
  ...
  if (!eric_events || eric_events->events().empty()) {
    return absl::nullopt;
  }
  // the index field used to build the uid together with the stream_id is retrieved from the
  // filterstate itself
  if (field_ == "INDEX") {
    return std::to_string(eric_events->getCurrentEventIndex());
  }
  if (max_length_) {
    std::string val = eric_events->getCurrentEvent().getEventField(field_);
    SubstitutionFormatUtils::truncate(val, max_length_);
    return val;
  }
  return eric_events->getCurrentEvent().getEventField(field_);
}

// Used by JSON and GRPC log formats (only %EVENT(INDEX)% uses format())
ProtobufWkt::Value EventFormatter::formatValue(const StreamInfo::StreamInfo& stream_info) const {
  const auto& filter_state = stream_info.filterState();
  auto eric_events =
      filter_state.getDataReadOnly<StreamInfo::EricEventState>(StreamInfo::EricEventState::key());
  ...
  // The index field is used to build the uid together with the stream_id which
  // is retrieved from the filterstate itself
  if (field_ == "INDEX") {
    return ValueUtil::numberValue(eric_events->getCurrentEventIndex());
  }
  // All other fields
  return eric_events->getCurrentEvent().getEventFieldValue(field_, max_length_);
}
          ...
          {"EVENT",
           {CommandSyntaxChecker::PARAMS_REQUIRED | CommandSyntaxChecker::LENGTH_ALLOWED,
            [](const std::string& format, absl::optional<size_t> max_length) {
              std::string field, alternative;
              SubstitutionFormatUtils::parseSubcommandHeaders(format, field, alternative);
              return std::make_unique<EventFormatter>(field, max_length);
            }}},
          {"DYNAMIC_METADATA",
