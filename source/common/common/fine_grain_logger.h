...
#define FINE_GRAIN_STREAM_UL_LOG(LEVEL, FORMAT, STREAM, ULID, ...)                                      \
  FINE_GRAIN_LOG(LEVEL, "[U{}][C{}][S{}] " FORMAT, ULID, (STREAM).connection() ?                        \
  (STREAM).connection()->id() : 0, (STREAM).streamId(), ##__VA_ARGS__)
