...
void setLogFormatForLogger(spdlog::logger& logger, const std::string& log_format) {
  ...
  // Escape ULID string when it sees "%U".
  formatter
...
void EscapeMessageULId::format(const spdlog::details::log_msg& msg, const std::tm&,
                               spdlog::memory_buf_t& dest) {
  absl::string_view payload = absl::string_view(msg.payload.data(), msg.payload.size());
  if (payload.rfind(Utility::TagsPrefix, 0) == std::string::npos) {
    return;
  }
  const auto tag_pos = payload.find(EscapeMessageULId::TagName, 0);
  if (tag_pos == std::string::npos) {
    return;
  }
  const auto tags_end_pos =
      payload.find(Utility::TagsSuffixForSearch) + Utility::TagsSuffixForSearch.size();

  const auto tags = absl::string_view(payload.data() + tag_pos - 1, tags_end_pos - tag_pos - 1);
  std::vector<absl::string_view> vector_tags = absl::StrSplit(tags, absl::ByAnyChar(",:"));
  if (vector_tags[1].size() < 2) {
    return;
  }
  dest.append(vector_tags[1].data() + 1, vector_tags[1].data() + vector_tags[1].size() - 1);
}

void EscapeMessageC::format(const spdlog::details::log_msg& msg, const std::tm&,
                            spdlog::memory_buf_t& dest) {
  ...
  const auto tag_pos = payload.find(EscapeMessageC::TagName, 0);
  if (tag_pos == std::string::npos) {
    return;
  }
  const auto tags_end_pos =
...
void EscapeMessageS::format(const spdlog::details::log_msg& msg, const std::tm&,
                            spdlog::memory_buf_t& dest) {
  absl::string_view payload = absl::string_view(msg.payload.data(), msg.payload.size());
  if (payload.rfind(Utility::TagsPrefix, 0) == std::string::npos) {
    return;
  }
  const auto tag_pos = payload.find(EscapeMessageS::TagName, 0);
  if (tag_pos == std::string::npos) {
    return;
  }
  const auto tags_end_pos =
