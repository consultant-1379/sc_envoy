...
#define ALL_LOGGER_IDS(FUNCTION)                                                                   \
  ...
  FUNCTION(envoy_bug)                                                                              \
  FUNCTION(eric_proxy)                                                                             \
  FUNCTION(eric_ingress_ratelimit)                                                                 \
  FUNCTION(eric_tap)                                                                               \
  FUNCTION(ext_authz)                                                                              \
  FUNCTION(ext_proc)                                                                               \
  FUNCTION(rocketmq)                                                                               \
...
/**
 * When added to a formatter, this adds 'U' as a user defined flag in the log pattern that adds a
 * Unique Logging Id 
 */
class EscapeMessageULId : public spdlog::custom_flag_formatter {
public:
  void format(const spdlog::details::log_msg& msg, const std::tm& tm,
              spdlog::memory_buf_t& dest) override;

  std::unique_ptr<custom_flag_formatter> clone() const override {
    return spdlog::details::make_unique<EscapeMessageULId>();
  }

  constexpr static char Placeholder = 'U';
private:
  constexpr static absl::string_view TagName = "UlId";
};

/**
 * When added to a formatter, this adds 'C' as a user defined flag in the log pattern that adds a
 * connection id
 */
class EscapeMessageC : public spdlog::custom_flag_formatter {
public:
  void format(const spdlog::details::log_msg& msg, const std::tm& tm,
              spdlog::memory_buf_t& dest) override;

  std::unique_ptr<custom_flag_formatter> clone() const override {
    return spdlog::details::make_unique<EscapeMessageC>();
  }

  constexpr static char Placeholder = 'K';

private:
  constexpr static absl::string_view TagName = "ConnectionId";
};


/**
 * When added to a formatter, this adds 'S' as a user defined flag in the log pattern that adds a
 * stream id
 */
class EscapeMessageS : public spdlog::custom_flag_formatter {
public:
  void format(const spdlog::details::log_msg& msg, const std::tm& tm,
              spdlog::memory_buf_t& dest) override;

  std::unique_ptr<custom_flag_formatter> clone() const override {
    return spdlog::details::make_unique<EscapeMessageS>();
  }

  constexpr static char Placeholder = 'Q';

private:
  constexpr static absl::string_view TagName = "StreamId";
};
...
#define ENVOY_STREAM_UL_LOG_TO_LOGGER(LOGGER, LEVEL, FORMAT, STREAM, ULID, ...)                    \
  do {                                                                                             \
    if (ENVOY_LOG_COMP_LEVEL(LOGGER, LEVEL)) {                                                     \
      std::map<std::string, std::string> log_tags;                                                 \
      log_tags.emplace("ConnectionId",                                                             \
...
#define ENVOY_STREAM_UL_LOG(LEVEL, FORMAT, STREAM, ULID, ...)                                      \
  ENVOY_STREAM_UL_LOG_TO_LOGGER(ENVOY_LOGGER(), LEVEL, FORMAT, STREAM, ULID, ##__VA_ARGS__);
