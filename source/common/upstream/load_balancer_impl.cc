...
void ZoneAwareLoadBalancerBase::analyzePrioritySet(LoadBalancerContext* context) {
  if (context && !priority_set_.hostSetsPerPriority().empty()) {
...
HostConstSharedPtr ZoneAwareLoadBalancerBase::chooseHost(LoadBalancerContext* context) {
  ...
  for (size_t i = 0; i < max_attempts; ++i) {
    ENVOY_LOG(debug, "Calling chooseHostOnce(), try={} max_attempts: {}", i, max_attempts);
    host = chooseHostOnce(context);
    if(host != nullptr) {
      ENVOY_LOG(debug, "Candidate host returned by chooseHostOnce() host {} (ip {})",
                        host->hostname(),host->address()->asStringView());
    }
    ...
    if(host != nullptr) {
      ENVOY_LOG(debug, "Candidate host {} (ip {}) not accepted", host->hostname(),
...
ZoneAwareLoadBalancerBase::hostSourceToUse(LoadBalancerContext* context, uint64_t hash) const {
  ENVOY_LOG(debug, "LoadBalancerBase: calling determinePriorityLoad() and then "
                   "choosePriority()  from inside the lb");
...
HostConstSharedPtr EdfLoadBalancerBase::chooseHostOnce(LoadBalancerContext* context) {
  ENVOY_LOG(debug, "EdfLoadBalancerBase::chooseHostOnce()");
