...
protected:
  ...
  std::pair<std::vector<uint32_t>,uint32_t> override_host_retry_indices = 
                                  std::make_pair<std::vector<uint32_t>,uint32_t>({0}, 0);
...
public:
  ...
  const Router::MetadataMatchCriteria* ericProxyMetadata() override { return nullptr; }
  ...
  std::vector<uint32_t> overrideHostRetryIndices() override { return {}; }

  uint32_t getOverrideHostRetryIndexToChoose() override { return 0; }

  void setOverrideHostRetryIndexChosen(const uint32_t&) override {}
};
...
public:
  ...
  void analyzePrioritySet(LoadBalancerContext* context) override;
