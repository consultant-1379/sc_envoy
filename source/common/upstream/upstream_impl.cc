...
HostDescriptionImpl::HostDescriptionImpl(
    ...
    : cluster_(cluster), hostname_(hostname),
      ...
      canary_(Config::Metadata::metadataValue(metadata.get(),
                  ...
                  .bool_value()),
      stats_per_nf_(cluster->statsScope(), metadata.get()),
...
// TODO(enaidev) :: Add another entry in hostmap which is just the fqdn of host without the
// port
void MainPrioritySetImpl::updateCrossPriorityHostMap(const HostVector& hosts_added,
  ...
  // Make the corssPriorityHostMap include fqdns as keys for hosts.
  // Hostname is enough here as hostname comes including the port from the manager
  // If port is absent in the config, the well known ones are used based on configured scheme
  for (const auto& host : hosts_removed) {
    mutable_cross_priority_host_map_->erase(addressToString(host->address()));
    // DND-48736 Ip version segregated cross priority host map
    // Erase host entries from the mutable cross priority hostmap
    mutable_cross_priority_host_map_->erase(absl::AsciiStrToLower(host->hostname()), host);
    ENVOY_LOG(debug, "Removing host {} (ip:{}) from hostmap", host->hostname(),
              host->address()->asStringView());
  }
  for (const auto& host : hosts_added) {
    mutable_cross_priority_host_map_->insert(addressToString(host->address()),host);
    // DND-48736 Adaptations to support a IP version segregated cross priority host map
    // HostMap is implemented as a refcounted map of vector of hosts
    if (!Http::Utility::parseAuthority(host->hostname()).is_ip_address_) {
      mutable_cross_priority_host_map_->insert(absl::AsciiStrToLower(host->hostname()), host);
    }
    ENVOY_LOG(debug, "Adding host {} (ip:{}) to hostmap", host->hostname(),
              host->address()->asStringView());
  }
}
...
//SC Minor impacts because HostMap is a RefCountedHashMap<key,vector<HostSharedPtr>>
bool BaseDynamicClusterImpl::updateDynamicHostList(
  ...
  for (const HostSharedPtr& host : new_hosts) {
    ...
    // (sc-dualstack) Since its an IP:port addr only check means if key is found in map, the host is
    // present at vector idx 0
    ...
    if (existing_host_found) {
      existing_host->second[0].first->healthFlagClear(Host::HealthFlag::PENDING_DYNAMIC_REMOVAL);
    }
    ...
    const bool health_check_address_changed =
         ...
         *existing_host->second[0].first->healthCheckAddress() != *host->healthCheckAddress());
    bool locality_changed = false;
    locality_changed =
        (existing_host_found &&
         (!LocalityEqualTo()(host->locality(), existing_host->second[0].first->locality())));
    if (locality_changed) {
      hosts_with_updated_locality_for_current_priority.emplace(existing_host->first);
    }
    // Added criterion (DND-30242):
    // If the new host is matched with an existing one but its hostname has changed, remake the host
    // as the current interface does not allow altering the hostname
    const bool hostname_changed =
        (existing_host_found && existing_host->second[0].first->hostname() != host->hostname());

    const bool active_health_check_flag_changed =
        (health_checker_ != nullptr && existing_host_found &&
         existing_host->second[0].first->disableActiveHealthCheck() !=
             host->disableActiveHealthCheck());
    ...
    const bool skip_inplace_host_update =
        health_check_address_changed || locality_changed || active_health_check_flag_changed  ||hostname_changed;
    ...
    if (existing_host_found && !skip_inplace_host_update) {
      ...
      if (existing_host->second[0].first->weight() != host->weight()) {
        existing_host->second[0].first->weight(host->weight());
      ...
      hosts_changed |= updateEdsHealthFlag(*host, *existing_host->second.at(0).first);

      // Did metadata change?
      bool metadata_changed = true;
      if (host->metadata() && existing_host->second[0].first->metadata()) {
        metadata_changed = !Protobuf::util::MessageDifferencer::Equivalent(
            *host->metadata(), *existing_host->second[0].first->metadata());
      } else if (!host->metadata() && !existing_host->second[0].first->metadata()) {
        metadata_changed = false;
      }

      if (metadata_changed) {
        // First, update the entire metadata for the endpoint.
        existing_host->second[0].first->metadata(host->metadata());
        ...
        existing_host->second[0].first->canary(host->canary());
      ...
      if (host->priority() != existing_host->second[0].first->priority()) {
        existing_host->second[0].first->priority(host->priority());
        hosts_added_to_current_priority.emplace_back(existing_host->second[0].first);
      }

      final_hosts.push_back(existing_host->second[0].first);
      ...
      if (health_checker_ != nullptr && !host->disableActiveHealthCheck()) {
        if (Runtime::runtimeFeatureEnabled(
          ...
          if (existing_host_found && !health_check_address_changed &&
            ...
            ASSERT(!existing_host->second.at(0).first->healthFlagGet(Host::HealthFlag::PENDING_ACTIVE_HC) ||
                   existing_host->second.at(0).first->healthFlagGet(Host::HealthFlag::FAILED_ACTIVE_HC));
            ASSERT(!existing_host->second.at(0).first->healthFlagGet(Host::HealthFlag::ACTIVE_HC_TIMEOUT) ||
                   existing_host->second.at(0).first->healthFlagGet(Host::HealthFlag::FAILED_ACTIVE_HC));
            ...
            const uint32_t existing_host_statuses = existing_host->second.at(0).first->healthFlagsGetAll();
