...
public:
  ...
  void metadata(MetadataConstSharedPtr new_metadata) override {
    auto& new_socket_factory = resolveTransportSocketFactory(address_, new_metadata.get());
    {
      ...
      // metadata mutex should block the thread and theoretically we can safely modify builtin
      stats_per_nf_.updateMeta(new_metadata.get());
    }
  }
  ...
  ClusterTrafficPerNfStats& statsPerNf() const override {
    absl::ReaderMutexLock lock(&metadata_mutex_);
    return stats_per_nf_;
  }
...
private:
  ...
  mutable ClusterTrafficPerNfStats stats_per_nf_ ABSL_GUARDED_BY(metadata_mutex_);
