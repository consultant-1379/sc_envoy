...
struct ActiveStreamDecoderFilter : public ActiveStreamFilterBase,
  ...
  void setViaHeaderContents(const std::vector<absl::string_view>&&) override;

  const std::vector<absl::string_view>& viaHeaderContents() const override;

  std::vector<uint32_t> getPreferredHostRetryIndices() override;

  void setPreferredHostRetryIndices(const std::vector<uint32_t>& ) override;
...
private:
  ...
  std::vector<absl::string_view> via_header_contents_;
  std::vector<uint32_t> preferred_host_retry_indices_{0};
