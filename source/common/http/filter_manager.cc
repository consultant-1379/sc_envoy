...
void ActiveStreamDecoderFilter::modifyDecodingBuffer(
  ...
  // DND-28513, we are modifying the body in router.cc:onUpstreamHostSelected()
...
void ActiveStreamDecoderFilter::requestDataTooLarge() {
  ...
  if (parent_.state_.decoder_filters_streaming_) {
    ...
    streamInfo().setResponseFlag(StreamInfo::ResponseFlag::PayloadTooLarge);
...
void ActiveStreamDecoderFilter::setViaHeaderContents(const std::vector<absl::string_view>&& from) {
  parent_.via_header_contents_ = from;
};


const std::vector<absl::string_view>& ActiveStreamDecoderFilter::viaHeaderContents() const {
  return parent_.via_header_contents_;
};

std::vector<uint32_t> ActiveStreamDecoderFilter::getPreferredHostRetryIndices() {
  return parent_.preferred_host_retry_indices_;
}

void ActiveStreamDecoderFilter::setPreferredHostRetryIndices(const std::vector<uint32_t>& vector) {
  parent_.preferred_host_retry_indices_.assign(vector.begin(),vector.end());
}
