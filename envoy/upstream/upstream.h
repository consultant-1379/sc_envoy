...
class RefCountHashMap : public absl::flat_hash_map<K, std::vector<std::pair<V, uint16_t>>> {
public:
  void insert(const K& key, const V& value) {
    auto iter = this->find(key);
    if (iter == this->end()) {
      std::vector<std::pair<V, uint16_t>> vec_val{std::make_pair(value, 1)};
      this->emplace(key, vec_val);
    } else {
      for (auto& entry :
           iter->second) { // vect_entry is a pair of (std::shared_ptr<Host>,ref_count)
        // If entry found on vector the increase ref-count
        if (absl::EqualsIgnoreCase((entry.first)->address()->asStringView(),
                                   value->address()->asStringView())) {
          entry.second++;
          return;
        }
      }
        iter->second.push_back(std::make_pair(value,1));
    }
  }
  ...
  void erase(const K& key, const V& value){
   auto iter = this->find(key);
   if(iter != this->end()){
     std::vector<std::pair<V, uint16_t>>& hosts = iter->second;
     for (typename std::vector<std::pair<V, uint16_t>>::iterator host_it = hosts.begin();
          host_it != hosts.end(); host_it++) {
       if (absl::EqualsIgnoreCase((host_it->first)->address()->asStringView(),
                                  value->address()->asStringView())) {
         if (--host_it->second == 0) {
           hosts.erase(host_it);
         }
  ...
  void erase(const K& key){
    auto iter = this->find(key);
    if(iter != this->end()) {
      std::vector<std::pair<V,uint16_t>>& vect = iter->second;
      if (--vect[0].second == 0) {
        Base::erase(iter);
      }
    }
  ...
  using Base = absl::flat_hash_map<K, std::vector<std::pair<V, uint16_t>>>;
};
...
using HostMap = Envoy::Upstream::RefCountHashMap<std::string,Upstream::HostSharedPtr>;
