...
public:
  ...
  /**
   * @return Router::MetadataMatchCriteria* metadata appended to the request from eric_proxy filter
   *
   */
  virtual const Router::MetadataMatchCriteria* ericProxyMetadata() PURE;
  ...
  /**
   * This is part of SC's custom reselection logic and eric_reselect_priorities priority retry
   * predicate. Finalize (=0) the retries on the preferred host via the overrideHost() functionality
   */
  virtual void finalizePreferredHostRetries();

  virtual void setSniFromHostName(const absl::string_view server_name);
  virtual void setAutoSanValidation(const absl::string_view server_name);
  ...
  /** 
   * Returns which overriden HostPtr should be selected if overrideHostToSelect() returns
   * multiple host entries in HostMap
   */
   virtual std::vector<uint32_t> overrideHostRetryIndices() PURE; 

   virtual void setOverrideHostRetryIndexChosen(const uint32_t&) PURE;

   virtual uint32_t getOverrideHostRetryIndexToChoose() PURE;
...
public:
  ...
  /**
   * Updates a priority retry predicate before a retry is decided to take place.
   * More specifically, used eric_reselect_priorities which encaptulates SC's custom relesection
   * logic A default implementation (no-op) can be found in load_balancer.cc
   * @param context supplies the context which is used in host selection.
   */
  virtual void analyzePrioritySet(LoadBalancerContext* context);
};
