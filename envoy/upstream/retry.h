...
public:
  ...
  /**
   * returns the configured number of preferred host retries left.
   */
  virtual uint32_t& preferredHostRetries() PURE;

  /**
   * returns true if there are available hosts for a retry to take place.
   * Currently only implemented by eric_reselect_priorities.
   */
...
public:
   ...
   * @param via_header_hosts contains the via header entries to enable fast lookups
