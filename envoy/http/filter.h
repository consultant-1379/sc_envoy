...
public:
  ...
  virtual void setViaHeaderContents(const std::vector<absl::string_view>&& parsed_cdns) PURE;

  virtual const std::vector<absl::string_view>& viaHeaderContents() const PURE;

  /**
   * @return The indices of hosts(ip:port endpoints) in hostmap in which preferred host has to be 
   * retried in preferred and strict routing 
   */
  virtual std::vector<uint32_t> getPreferredHostRetryIndices() PURE;

  /**
   * @param idx Set the indices of hosts(ip:port endpoints) in hostmap in which preferred 
   * host has to be retried
   * */
  virtual void setPreferredHostRetryIndices(const std::vector<uint32_t>& vector) PURE;
};
